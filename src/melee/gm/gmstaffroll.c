...
{
    return gm_IsCharacterUnlocked(arg0);
}

bool gm_801AA688(void)
{
    PAD_STACK(4);
    return gm_IsCharacterUnlocked(CKIND_MARS) ||
            ...
            for (j = 0; j < 6; j++) {
                ...
                if (j == 0) {
                    ...
                    if (selected == 0x4B && j == 2) {
                        line_num = gm_IsCharacterUnlocked(0x16U) * 2;
                        HSD_SisLib_803A6368(
                            text_arr[j],
                            0xBEC + gm_IsCharacterUnlocked(7U) + line_num);
