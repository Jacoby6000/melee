...
void gm_801B4064(GameScene* arg0)
{
    ...
    temp_r27->x0.spawn_dir = temp_r31->x9;
...
void gm_801B4C5C(GameScene* scene)
{
    ...
    {
        gm_SetPendingSceneToSuccessorOf(
...
void gm_801B4D34(GameScene* scene)
{
     ...
     */
    if (gm_IsCharacterUnlocked(CKIND_FALCO) && HSD_Randi(100) <= 33) {
...
void gm_801B4EB8(GameScene* scene)
{
    ...
    if (!gm_IsCharacterUnlocked(CKIND_LUIGI)) {
        for (i = 0; i < 3; i++) {
